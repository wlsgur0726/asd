...
namespace asd
{
	...
	public:
		...
		// '\0'을 제외한 캐릭터 수를 리턴
		inline size_t GetLength() const asd_noexcept
		{
		...
		// 문자열의 시작 포인터를 리턴
		inline CharType* GetData() const asd_noexcept
		{
		...
		inline operator SupportType_StdString() const asd_noexcept
		{
		...
		inline void Resize(IN size_t a_newLen,
						   IN bool a_preserveOldData = false) asd_noexcept
		{
			if (a_newLen == 0) {
...
#define asd_String_Define_CompareOperator(CompareFunction, TemplateType, CaseSensitive_DefaultVar)		\
		...
		inline int Compare(IN TemplateType a_rval,														\
						   IN bool a_caseSensitive = CaseSensitive_DefaultVar) const asd_noexcept		\
		{																								\
			return CompareFunction(GetData(), a_rval, a_caseSensitive);									\
		}																								\
																										\
		inline bool operator == (IN TemplateType a_rval) const asd_noexcept								\
		{																								\
...
#define asd_String_Define_CompareFunction(CharTypePointer_LeftVar,										\
		...
		inline static int Compare(IN TemplateType a_templateVar,										\
								  ...
								  IN bool a_caseSensitive = CaseSensitive_DefaultVar) asd_noexcept		\
		{																								\
		...
		inline static int Compare(IN const CharType* CharTypePointer_LeftVar,							\
								  ...
								  IN bool IgnoreCaseVar = CaseSensitive_DefaultVar) asd_noexcept		\



		// 대입연산자(=)를 정의하면서 생성자까지 한번에 정의하는 매크로.
#define asd_String_Define_AssignmentOperator_Substitute(ArgType, ArgVar)								\
		BasicString(IN ArgType ArgVar) asd_noexcept														\
		{																								\
			*this = ArgVar;																				\
		}																								\
																										\
		inline ThisType& operator = (IN ArgType ArgVar) asd_noexcept									\



		// += 연산자를 정의하면서 << 와 + 연산자를 한번에 정의하는 매크로.
#define asd_String_Define_AssignmentOperator_Append(ArgType, ArgVar)									\
		inline ThisType operator + (IN ArgType ArgVar) const asd_noexcept								\
		{																								\
			ThisType ret;																				\
			ret << *this << ArgVar;																		\
			return ret;																					\
